@echo off
echo Bienvenu dans la creation des tables, vues, VM, fonctions, sequences et declencheurs du projet h de correction de la Base Voie !
...
copy /b %chemin_code_sequence%\creation_seq_temp_i_troncon_objectid.sql + ^
%chemin_code_table%\creation_temp_i_agent.sql + ^
%chemin_code_table%\creation_temp_i_libelle.sql + ^
%chemin_code_table%\creation_temp_i_type_voie.sql + ^
%chemin_code_table%\creation_temp_i_rivoli.sql + ^
%chemin_code_table%\creation_temp_i_voie_physique.sql + ^
%chemin_code_table%\creation_temp_i_voie_administrative.sql + ^
%chemin_code_table%\creation_temp_i_troncon.sql + ^
%chemin_code_table%\creation_temp_i_relation_voie_physique_administrative.sql + ^
%chemin_code_table%\creation_temp_i_hierarchisation_voie.sql + ^
%chemin_code_table%\creation_temp_i_voie_lateralite.sql + ^
%chemin_code_trigger%\creation_b_iux_temp_i_troncon_date_pnom.sql + ^
%chemin_code_temp%\desactivation_contraintes_index_tables_projet_i.sql ^
%chemin_code_temp%\temp_code_ddl_schema_transitoire_projet_i.sql

:: 3. lancement de SQL plus.
::CD C:/ora12c/R1/BIN

:: 4. Execution de sqlplus. pour lancer les requetes SQL.
::sqlplus.exe %USER%/%MDP%@%INSTANCE% @%chemin_code_temp%\temp_code_ddl_schema_transitoire_projet_i.sql
