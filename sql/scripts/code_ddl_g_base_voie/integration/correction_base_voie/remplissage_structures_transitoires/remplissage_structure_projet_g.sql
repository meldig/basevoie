...
-- Mise à jour de la séquence d'incrémentation de la PK de TEMP_G_LIBELLE
SELECT
    MAX(objectid)+1
FROM
    G_BASE_VOIE.TEMP_G_LIBELLE;

ALTER TABLE G_BASE_VOIE.TEMP_G_LIBELLE MODIFY objectid NUMBER(38,0) GENERATED BY DEFAULT AS IDENTITY(START WITH 4 INCREMENT BY 1);
--

INSERT INTO G_BASE_VOIE.TEMP_G_LIBELLE(libelle_court, libelle_long)
...
WITH
    C_1 AS(
        ...
        FROM
          ...
          INNER JOIN G_BASE_VOIE.TEMP_F_RELATION_TRONCON_VOIE_PHYSIQUE b ON b.fid_troncon = a.objectid
    )
...
-- 49713
-- Sélection du nombre de tronçons dans TEMP_G_TRONCON
SELECT
    COUNT(objectid)
FROM
...
WHERE
    ...
    AND a.cdvalvoi = 'V'
    AND c.cvalide = 'V'
    AND CAST(a.ccomvoi AS NUMBER(38,0))  NOT IN(SELECT objectid FROM G_BASE_VOIE.TEMP_G_VOIE_ADMINISTRATIVE);
*/
