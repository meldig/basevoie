-- VM_AUDIT_SEUIL_TRONCON_NON_JOINTIF: Troncon d'une meme voie non jointif

-- 0. Suppression de l'ancienne vue matérialisée
/*
DROP INDEX VM_AUDIT_SEUIL_TRONCON_NON_JOINTIF_SIDX;
DELETE FROM USER_SDO_GEOM_METADATA WHERE TABLE_NAME = 'VM_AUDIT_SEUIL_TRONCON_NON_JOINTIF';
DROP MATERIALIZED VIEW VM_AUDIT_SEUIL_TRONCON_NON_JOINTIF;
*/

-- 1. Creation vue
CREATE MATERIALIZED VIEW G_BASE_VOIE.VM_AUDIT_SEUIL_TRONCON_NON_JOINTIF (identifiant, code_troncon, code_seuil, statut_connection, geom)
REFRESH ON DEMAND
FORCE
...
FROM
    ...
    INNER JOIN G_BASE_VOIE.TEMP_ILTASEU d ON d.idseui = c.idseui,
...
-- 2. Clé primaire
ALTER MATERIALIZED VIEW G_BASE_VOIE.VM_AUDIT_SEUIL_TRONCON_NON_JOINTIF
